...
void free_tree(struct node* root) {                         //функция освобождения памяти дерева
    if (root != NULL) {                                     //проверка, не пусто ли дерево
        free_tree(root->left);                              //рекурсивный вызов для левого поддерева
        free_tree(root->right);                             //рекурсивный вызов для правого поддерева
