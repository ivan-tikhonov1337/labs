...
data procedure(udt *d) {                      // Находит максимальный элемент в деке и удаляет его, перемещая все остальные элементы влево или вправо,
    data t, a;                                // чтобы сохранить порядок элементов. Затем он возвращает максимальный элемент.
    int max = -INF - 1;
    int size = d->size;
...
udt* sort(udt *d) {                           // Использует procedure, чтобы находить максимальный элемент и добавлять его
    ...
    data t;                                   // содержащий все элементы в порядке убывания.
    for (int i = 0; i < size; i++) {
